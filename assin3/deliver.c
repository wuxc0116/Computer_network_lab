...
int main(int argc, char **argv)
{
	...
	double SAMPLE_RTT, RTT_DEV, RTO;
	SAMPLE_RTT = RTT;
	RTT_DEV = RTT / 2;
	RTO = SAMPLE_RTT + 4 * RTT_DEV;
	...
	while(curr_packet != NULL){
		...
		if(recv_n = recvfrom(sock_fd, buffer, BUFFER_SIZE, 0, (struct sockaddr *) &serv_addr, &addr_len) == -1)
        ...
        if(isRetransmission){
            ...
            RTT_DEV = (1 - BETA)*RTT_DEV + BETA * fabs(SAMPLE_RTT - RTT);
            SAMPLE_RTT = (1 - ALPHA)*SAMPLE_RTT + ALPHA * RTT;
            RTO = SAMPLE_RTT + 4 * RTT_DEV;
            if(RTO < 1){
                RTO = 1;
            }
